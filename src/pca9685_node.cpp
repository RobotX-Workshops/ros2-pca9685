...
namespace pca9685
{
      ...
      : Node("pca9685_node", rclcpp::NodeOptions(options).enable_logger_service(true))
  {
    ...
    std::vector<bool> enabled_channels = this->declare_parameter<std::vector<bool>>("enabled_channels", default_channels);

    // Validate channel configuration
    if (enabled_channels.size() != 16)
    {
      RCLCPP_WARN(this->get_logger(), "enabled_channels parameter must have exactly 16 boolean values. Using default (all enabled).");
    ...
    RCLCPP_INFO(this->get_logger(), "Subscribed to %d out of %d PWM channels.", active_channels, number_of_channels);
  }
